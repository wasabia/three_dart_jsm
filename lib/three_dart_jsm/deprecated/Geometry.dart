...
class Geometry with THREE.EventDispatcher {
  ...
  applyMatrix4( matrix ) {
	...
	fromBufferGeometry( geometry ) {
		...
		for ( var i = 0; i < position.count; i ++ ) {
			...
			scope.vertices.add( new THREE.Vector3.init().fromBufferAttribute( position, i ) );

			if ( color != null ) {

				scope.colors.add( new THREE.Color(0,0,0).fromBufferAttribute( color, i ) );
    ...
    var changes = List.filled(this.vertices.length, 0);
		...
		for ( var i = 0, il = this.vertices.length; i < il; i ++ ) {
			...
			var key = '${THREE.Math.round( v.x * precision )}_${THREE.Math.round( v.y * precision )}_${THREE.Math.round( v.z * precision )}';
  ...
  static createBufferGeometryFromObject( object ) {
		...
		if ( object.isPoints || object.isLine ) {
			...
			var positions = new THREE.Float32BufferAttribute( geometry.vertices.length * 3, 3, false );
			var colors = new THREE.Float32BufferAttribute( geometry.colors.length * 3, 3, false );
			...
			if ( geometry.lineDistances && geometry.lineDistances.length == geometry.vertices.length ) {
				...
				var lineDistances = new THREE.Float32BufferAttribute( geometry.lineDistances.length, 1, false );
